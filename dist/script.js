...
document.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_sliders__WEBPACK_IMPORTED_MODULE_2__["default"])('.feedback-slider-item', 'horisontal', '.main-prev-btn', '.main-next-btn');
  ...
  Object(_modules_checkTextInputs__WEBPACK_IMPORTED_MODULE_5__["default"])('[name="message"]'); //простой способ показа элементов на странице
...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener("submit", function (e) {
      ...
      statusMessage.classList.add("status"); //заменяем контент формы

      item.parentNode.appendChild(statusMessage); //скрываем саму форму

      item.classList.add('animated', 'fadeOutUp');
      setTimeout(function () {
        item.style.display = "none";
      }, 200); //создаем новый контент вместо данных формы
      ...
      statusMessage.appendChild(textMessage); //создаем переменную для отправки данных с формы

      var formData = new FormData(item);
      var api;
      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question;
      console.log(api); //передаем данные на сервер
...
var modals = function modals() {
  ...
  function calcScroll() {
    ...
    div.remove();
    return scrollWidth;
  } //настраиваем отткрытие модального окна при скролле до конца документа
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); //простая реализация показа элементов на странице
